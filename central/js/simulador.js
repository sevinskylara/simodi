...
var Simulador = (function () {
  ...
  /* caudal en mL/kg/h · temp en °C · color: nivel 1-8 o RGB fijo */
  var ESCENARIOS = {
    normal: {
      nombre: 'Evolución normal',
      resumen: 'Diuresis conservada 1–1,5 mL/kg/h, orina ámbar claro, afebril.',
      caudal: function (h) { return 1.15 + 0.22 * Math.sin(h / 1.9); },
      temp:   function (h) { return 36.8 + 0.12 * Math.sin(h / 3.1); },
      nivel:  function (h) { return 3.6 + 0.5 * Math.sin(h / 4.0); }
    },
    deshidratacion: {
      nombre: 'Deshidratación progresiva',
      resumen: 'El caudal cae y la orina se concentra: la escala de color sube a 7–8.',
      caudal: function (h) { return 0.55 * Math.exp(-h / 10) + 0.35; },
      temp:   function (h) { return 37.2 + 0.1 * Math.sin(h / 2.5); },
      nivel:  function (h) { return Math.min(8, 4.6 + h * 0.33); }
    },
    oliguria_lra: {
      ...
      nivel:  function (h) { return Math.min(8, 5.4 + h * 0.2); }
    },
    sepsis: {
      ...
      nivel:  function (h) { return Math.min(8, 5.2 + h * 0.25); }
    },
    poliuria: {
      ...
      nivel:  function (h) { return 1.4 + 0.3 * Math.sin(h / 2); }
    },
    hematuria: {
      nombre: 'Hematuria post-quirúrgica',
      resumen: 'Diuresis normal pero orina roja que va aclarando con el lavado vesical.',
      caudal: function (h) { return 1.35 + 0.2 * Math.sin(h / 1.5); },
      temp:   function (h) { return 37.0 + 0.1 * Math.sin(h / 3); },
      rgb:    function (h) {
        var t = U.clamp(h / 14, 0, 1);          // de rojo franco a rosado
        return U.mezcla('#B0413E', '#D9A0A0', t);
      }
    },
    obstruccion: {
    ...
    bolsa_llena: {
      ...
      nivel:  function (h) { return 3.2; },
      sinVaciadoAuto: true
    },
    postquirurgico: {
      nombre: 'Post-operatorio en recuperación',
      resumen: 'Oliguria de las primeras horas que se normaliza con la reposición.',
      caudal: function (h) { return h < 3 ? 0.42 + h * 0.05 : Math.min(1.25, 0.57 + (h - 3) * 0.16); },
      temp:   function (h) { return 36.4 + Math.min(0.5, h * 0.08); },
      nivel:  function (h) { return Math.max(3, 5.5 - h * 0.25); }
    },
    enlace_intermitente: {
  ...
  function esc(d) { return ESCENARIOS[d.escenario] || ESCENARIOS.normal; }
  ...
  function generar(d, p, t, precarga) {
    ...
    if (d._volSim > cap * 0.97 && !e.sinVaciadoAuto) {
      d._volSim = 12;   // enfermería vació el colector (queda un resto)
    }
    if (d._volSim > cap) d._volSim = cap;   // rebalsa: la balanza se satura

    /* --- masa que ve la celda de carga (con ruido y cuantización) --- */
    var med = Modelo.estado.medicion;
    var pesoG = med.taraBolsaG + d._volSim * med.densidadOrina + U.ruido(0.35);
    pesoG = Math.round(pesoG / med.resolucionPesoG) * med.resolucionPesoG;
    ...
    if (!precarga && e.cortes) {
      if (t > d._cortadoHasta && t > d._proxCorte && !d._enCorte) {
        d._enCorte = true;
        d._cortadoHasta = t + (40 + Math.random() * 50) * 1000;
        d.estado = 'sin-senal';
        Modelo.registrarEvento(camaDe(d), 'enlace', 'Enlace perdido con ' + d.serie, d.id);
      }
      if (d._enCorte) {
        if (t < d._cortadoHasta) {
          d._bufferSim.push(lectura);          // se acumula en la memoria del ESP32
          return null;
        }
        // Reconexión: se vuelca el búfer marcado como reconstruido.
        d._enCorte = false;
        d.estado = 'en-linea';
        d._proxCorte = t + (6 + Math.random() * 8) * 60000;
        var n = d._bufferSim.length;
        d._bufferSim.forEach(function (l) { l.origen = 'buffer'; Modelo.ingresarMuestra(d.id, l); });
        d._bufferSim = [];
        Modelo.registrarEvento(camaDe(d), 'enlace',
          'Enlace restablecido con ' + d.serie + ' · ' + n + ' muestras recuperadas del búfer', d.id);
      }
    }
  ...
  function camaDe(d) { var c = Modelo.camaDeDispositivo(d.id); return c ? c.id : null; }

  /* Rellena `horas` de historial hacia atrás, a 1 muestra por minuto. */
  function precargar(d, p, horas) {
    var ahora = Date.now();
    d.inicioEscenario = ahora - horas * 3600000;
    d._volSim = 0; d._tPrev = d.inicioEscenario; d._bufferSim = [];
    d._cortadoHasta = 0; d._proxCorte = ahora + 5 * 60000;
    d.reloj = d.inicioEscenario;
    for (var t = d.inicioEscenario; t <= ahora; t += 60000) {
      generar(d, p, t, true);
    }
    d.reloj = ahora;
    d.estado = 'en-linea';
  ...
  function paso(dtRealMs) {
    var vel = Modelo.estado.velocidad || 1;
    Modelo.estado.camas.forEach(function (c) {
      if (!c.dispositivoId) return;
      var d = Modelo.estado.dispositivos[c.dispositivoId];
      if (!d || d.tipo !== 'sim') return;
      if (Modelo.estado.modo === 'real' && !Modelo.estado.mostrarPilotoEnReal) return;
      var p = c.pacienteId ? Modelo.estado.pacientes[c.pacienteId] : null;
      var t = (d.reloj || Date.now()) + dtRealMs * vel;
      d.reloj = t;
      generar(d, p, t, false);
    });
  }
  ...
  function listaEscenarios() {
    return Object.keys(ESCENARIOS).map(function (k) {
      return { clave: k, nombre: ESCENARIOS[k].nombre, resumen: ESCENARIOS[k].resumen };
    });
  }
  ...
  function cambiarEscenario(dispId, clave, precargaHoras) {
    ...
    var p = (c && c.pacienteId) ? Modelo.estado.pacientes[c.pacienteId] : null;
    d.escenario = clave;
    if (precargaHoras) {
      d.muestras = []; d.volTotalMl = 0;
      precargar(d, p, precargaHoras);
    } else {
      d.inicioEscenario = d.reloj || Date.now();
