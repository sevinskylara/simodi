...
var UI = (function () {
  ...
  var camaAbierta = null;
  var rangoHoras = 6;
  var tabDetalle = 'tendencias';
  var codigoSilenciar = null;
  var filtroTexto = '';
  ...
  function pintarMural() {
    ...
    if (!filas.length) {
      ...
      filas.forEach(function (f) {
        mural.appendChild(tarjetaCama(f.cama, f.m, f.estado));
      });
    }
  ...
  function tarjetaCama(cama, m, estado) {
    ...
    if (estado === 'sindispositivo') {
      var encabezado = m.paciente
        ? '<div class="cama-ident">' +
            ...
            '<div><div class="cama-nombre">' + U.esc(m.paciente.nombre) + '</div></div>' +
          '</div>'
        : '<span class="tag-cama">' + U.esc(cama.etiqueta) + '</span>';
      ...
      var textoBoton = cama.dispositivoId
        ? 'Reasignar'
        : (m.paciente ? 'Vincular dispositivo' : 'Asignar paciente');
    ...
    var claseBat =
      bat <= CFG.umbrales.bateriaCritica
        ? 'critica'
        : (bat <= CFG.umbrales.bateriaBaja ? 'baja' : '');

    var conectado = d.estado === 'en-linea';

    var alertasCama = Alertas.porCama(cama.id)
    ...
    div.innerHTML =
      '<div class="cama-head">' +
        ...
        '<div class="cama-badges">' +
          '<span class="badge ' + (d.tipo === 'sim' ? 'sim' : 'real') + '">' +
            (d.tipo === 'sim' ? 'PILOTO' : 'REAL') +
          '</span>' +
          '<span class="bateria ' + claseBat + '" title="Batería ' + Math.round(bat) + ' %' +
            (conectado ? ' · dispositivo conectado' : ' · dispositivo sin señal') + '">' +
      ...
      '<div class="cama-cuerpo">' +
        '<div class="dato-principal">' +
          '<div>' +
            '<div class="diuresis-val">' +
              '<span class="num">' + (m.mlKgH === null ? '—' : U.num(m.mlKgH, 2)) + '</span>' +
              '<span class="uni">mL/kg/h</span>' +
            '</div>' +
        ...
        '<div class="mini-datos">' +
          '<div class="mini' + claseTempMini(m.tempC) + '">' +
            '<div class="mini-val">' +
              (m.tempC === null ? '—' : U.num(m.tempC, 1)) +
              '<small>°C</small>' +
            '</div>' +
          '</div>' +
          '<div class="mini' + (m.color && m.color.estado !== 'ok' ? ' alerta' : '') + '">' +
            '<div class="mini-val">' +
              '<span class="mini-val-txt">' +
                (m.color ? U.esc(m.color.nombre) : '—') +
              '</span>' +
              '<span class="muestra-color" style="background:' +
                (m.color ? m.color.hex : '#333') +
              '"></span>' +
            '</div>' +
          '</div>' +
        '</div>' +
        '<div class="barra-bolsa">' +
          '<div class="rot">' +
            '<span>Bolsa</span>' +
            '<span>' + U.num(m.volBolsaMl, 0) + ' / ' + CFG.medicion.capacidadBolsaML + ' mL</span>' +
          '</div>' +
          '<div class="barra-pista">' +
            '<div class="barra-relleno' +
              (m.llenado >= CFG.umbrales.bolsaCritica
                ? ' critico'
                : (m.llenado >= CFG.umbrales.bolsaAviso ? ' aviso' : '')) +
              '" style="width:' + Math.round(m.llenado * 100) + '%"></div>' +
          '</div>' +
        '</div>' +
        (alertasCama.length
          ? '<div class="cama-alertas">' + alertasCama.map(pastillaAlerta).join('') + '</div>'
          : '') +
        (m.muestrasBuffer
          ? '<div class="cama-buffer-nota" title="Datos reconstruidos del búfer del equipo al reconectar"><span class="badge buffer">BÚFER</span> Datos reconstruidos al reconectar</div>'
          : '') +
      '</div>';
  ...
  function colorEstado(estado) {
    return {
      ...
      aviso: getCss('--aviso'),
      grave: getCss('--grave'),
      critico: getCss('--critico'),
      sinsenal: getCss('--neutro')
    }[estado] || getCss('--teal');
  }
  ...
  function pintarAlertas() {
    ...
    var peor = NIVEL_ORDEN.find(function (n) {
      return conteos[n] > 0;
    });

    NIVEL_ORDEN.forEach(function (n) {
      panel.classList.toggle('nivel-' + n, n === peor);
    });

    var el = U.$('#listaAlertas');

    if (!lista.length) {
      el.innerHTML = '<div class="vacio">Sin alertas activas. Todo dentro de rango.</div>';
      return;
    }
    ...
    lista.forEach(function (a) {
      ...
      var etiquetaSilencio = silInfo
        ? (silInfo.motivo === 'atendido' ? 'paciente atendido' : 'en espera')
        : '';

      var it = U.el(
        'div',
        'item-alerta ' + a.nivel + (a.reconocida ? ' reconocida' : '')
      );
      ...
      it.innerHTML =
        ...
        '<div class="alerta-cuerpo">' +
          '<div class="alerta-titulo">' +
            '<span class="cama-ref">' + U.esc(a.ref) + '</span>' +
            U.esc(a.titulo) +
          '</div>' +
          '<div class="alerta-detalle">' + U.esc(a.detalle) + '</div>' +
          '<div class="alerta-tiempo">' +
            U.desde(a.desde) +
            (a.reconocida ? ' · silenciada (' + etiquetaSilencio + ')' : '') +
          '</div>' +
          (a.reconocida
            ? ''
            : '<button class="btn-silenciar" data-codigo="' +
              U.esc(a.codigo) +
              '" data-titulo="' +
              U.esc(a.titulo) +
              '">Silenciar</button>') +
  ...
  function pintarEventos() {
    var ev = Modelo.estado.eventos
      .slice(-(eventosExpandido ? 300 : 40))
      .reverse();
    ...
    el.innerHTML = ev.map(function (e) {
      ...
      return '<div class="item-evento' + (reciente ? ' silencio-reciente' : '') + '">' +
        '<span class="hora">' + U.hora(e.t) + '</span>' +
        '<span>' +
          (cama
            ? '<b style="color:var(--texto)">' + U.esc(cama.etiqueta) + '</b> · '
            : '') +
          (e.operador
            ? '<span class="ev-operador">' + U.esc(e.operador) + '</span> — '
            : '') +
          U.esc(e.texto) +
        '</span>' +
      '</div>';
  ...
  function pintarBarraEstado() {
    ...
    if (!modoReal) {
      ...
      if (e.estado === 'conectado') {
        ...
        det.textContent =
          ...
          e.recibidas +
          ' muestras' +
          (e.recuperadas ? ', ' + e.recuperadas + ' recuperadas del búfer' : '') +
          ').';
    ...
    var bufferPend = Object.keys(Modelo.estado.dispositivos).reduce(function (n, k) {
      ...
      return n + (
        d.muestras.length
          ? d.muestras.filter(function (m) {
              return m.origen === 'buffer' && m.t > Date.now() - 120000;
            }).length
          : 0
      );
    }, 0);
    ...
    if (bufferPend) {
      U.$('#chipBuffer').querySelector('.chip-txt').textContent =
        bufferPend + ' recuperadas del búfer';
    }
  ...
  function filasDelMural() {
    ...
    hijos.forEach(function (el) {
      ...
      var fila = filas.filter(function (f) {
        return Math.abs(f.top - top) < 1;
      })[0];

      if (!fila) {
        fila = { top: top, height: 0 };
        filas.push(fila);
      }
  ...
  function igualarAlturasLateral() {
    ...
    if (eventosExpandido) {
      ...
      alturaEventos = Math.max.apply(
        null,
        filas.map(function (f) { return f.height; })
      );
    } else {
  ...
  function abrirDetalle(camaId) {
    ...
    U.$$('.tab-det').forEach(function (b) {
      b.classList.toggle('activo', b.dataset.tab === tabDetalle);
    });

    U.$$('.tab-panel').forEach(function (p) {
      p.classList.toggle('activo', p.dataset.panel === tabDetalle);
    });
  ...
  function renderDetalle() {
    ...
    U.$('#detNombre').textContent =
      m.paciente
        ? m.paciente.nombre
        : 'Cama sin paciente';

    U.$('#detMeta').textContent =
      m.paciente
        ? (
            m.paciente.hc +
            ' · ' +
            (m.paciente.edad ? m.paciente.edad + ' años · ' : '') +
            m.paciente.sexo +
            ' · ' +
            m.paciente.pesoKg +
            ' kg · ' +
            (m.paciente.dx || 'sin diagnóstico cargado')
          )
        : (
            m.dispositivo
              ? m.dispositivo.serie + ' sin paciente vinculado'
    ...
    if (m.dispositivo) {
      ...
      U.$('#tendenciasVacio').textContent =
        cama.dispositivoId
          ? 'El dispositivo vinculado no tiene datos disponibles en este equipo.'
          : (
              m.paciente
                ? 'Vinculá un dispositivo a esta cama para ver sus tendencias.'
                : 'Asigná un paciente y vinculá un dispositivo para ver tendencias.'
            );

      U.$('#detDispositivo').innerHTML =
        '<p class="nota">Esta cama todavía no tiene un dispositivo vinculado.</p>';
  ...
  function resVal(rot, val, sub, clase) {
    return '<div class="res ' + (clase || '') + '">' +
      '<div class="res-rot">' + U.esc(rot) + '</div>' +
      '<div class="res-val">' + val + '</div>' +
      '<div class="res-sub">' + U.esc(sub || '') + '</div>' +
    '</div>';
  }
  ...
  function renderResumen(m) {
    ...
    var kdigoTxt =
      ['Sin criterio KDIGO', 'KDIGO 1', 'KDIGO 2', 'KDIGO 3'][m.kdigo];
    ...
    el.innerHTML =
      resVal(
        'Diuresis (1 h)',
        m.mlH === null ? '—' : U.num(m.mlH, 0) + ' <small>mL/h</small>',
        (m.mlKgH !== null
          ? U.num(m.mlKgH, 2) + ' mL/kg/h'
          : 'sin peso cargado') +
        (m.umbralMlH !== null
          ? ' · umbral ' + U.num(m.umbralMlH, 0) + ' mL/h'
          : ''),
        claseTasa(m.mlKgH)
      ) +
      ...
      resVal(
        ...
        m.mlH6 === null
          ? '—'
          : U.num(m.mlKgH6, 2) + ' <small>mL/kg/h</small>',
        'promedio de las últimas 6 h',
        claseTasa(m.mlKgH6)
      ) +
      ...
      resVal(
        ...
        m.tempC === null
          ? '—'
          : U.num(m.tempC, 1) + ' <small>°C</small>',
        'máx. 6 h: ' +
          (m.tempMax6h !== null
            ? U.num(m.tempMax6h, 1) + ' °C'
            : '—'),
        claseTemp(m.tempC)
      ) +
      ...
      resVal(
        'Color',
        m.color ? U.esc(m.color.nombre) : '—',
        m.color ? m.color.detalle : '',
        m.color ? m.color.estado : ''
      ) +
      ...
      resVal(
        ...
        kdigoTxt,
        m.horasOliguria
          ? m.horasOliguria + ' h por debajo de 0,5 mL/kg/h'
          : 'diuresis dentro de objetivo',
        kdigoClase
      );
  }
  ...
  function renderTendencias(m) {
    ...
    var ms = d.muestras.filter(function (x) {
      return x.t >= desde;
    });

    if (!ms.length) {
      ms = d.muestras.slice(-2);
    }
    ...
    var horasDisponibles =
      d.muestras.length
        ? (ahora - d.muestras[0].t) / 3600000
        : 0;

    if (horasDisponibles < rangoHoras - 0.25) {
      notaRango.hidden = false;
      notaRango.textContent =
        'Solo hay ' +
        U.duracion(horasDisponibles * 3600000) +
        ' de historial en este equipo';
    } else {
    ...
    Graf.barrasDiuresis(
      U.$('#gDiuresis'),
      Modelo.bucketsHorarios(d, rangoHoras),
      {
        pesoKg: p ? p.pesoKg : null,
        umbralMlH: m.umbralMlH,
        umbralPoliuriaMlH: m.umbralPoliuriaMlH
      }
    );
    ...
    Graf.serie(
      ...
      ms.map(function (x) {
        return {
          t: x.t,
          v: x.volTotalMl - baseVolVentana,
          buffer: x.origen === 'buffer'
        };
      }),
      {
        color: getCss('--teal'),
    ...
    U.$('#legVolumen').textContent =
      U.num(m.volTotalMl - baseVolVentana, 0) +
      ' mL en las últimas ' +
      rangoHoras +
      ' h';
    ...
    Graf.serie(
      ...
      }),
      {
        ...
        unidad: ' °C',
        banda: {
          desde: CFG.umbrales.tempFebril,
          hasta: 42,
          color: hexAlpha(getCss('--critico'), .08)
        },
        min: 34.5,
        max: 40
      }
  ...
  function renderDispositivo(m) {
    ...
    var html =
      '<div class="fila">' +
        resVal(
          'N.º de serie',
          '<span style="font-family:var(--mono)">' + U.esc(d.serie) + '</span>',
          d.tipo === 'sim' ? 'dispositivo piloto (simulado)' : 'dispositivo real',
          ''
        ) +
        resVal(
          'Estado del enlace',
          d.estado === 'en-linea' ? 'Transmitiendo' : 'Sin señal',
          U.desde(d.ultimoContacto || d.reloj),
          d.estado === 'en-linea' ? 'ok' : 'critico'
        ) +
      '</div>' +
      ...
      '<div class="fila-3">' +
        resVal(
          'Batería',
          Math.round(d.bat) + ' <small>%</small>',
          duracionBateria(d),
          d.bat <= CFG.umbrales.bateriaBaja ? 'critico' : 'ok'
        ) +
        resVal(
          'Fuente',
          d.fuente === 'red' ? 'Toma eléctrica' : 'Batería',
          d.fuente === 'red' ? 'enchufado' : '',
          ''
        ) +
        resVal(
          'Señal (RSSI)',
          Math.round(d.rssi) + ' <small>dBm</small>',
          d.tipo === 'sim'
            ? 'simulado'
            : (e ? e.tipo : '—'),
          ''
    ...
    if (d.tipo === 'sim') {
      ...
      html +=
        '<div class="sep"></div>' +
        '<div class="subtitulo">Escenario piloto</div>' +
        '<p class="nota">' +
          '<b style="color:var(--texto)">' +
            U.esc(esc ? esc.nombre : d.escenario) +
          '</b> — ' +
          U.esc(esc ? esc.resumen : '') +
        '</p>' +
        '<div class="campo">' +
          '<label>Cambiar escenario</label>' +
          '<select id="selEscenarioDet"></select>' +
        '</div>' +
        '<div class="acciones">' +
          '<button class="btn secundario chico" id="btnAplicarEscenario">Aplicar (mantiene el historial)</button>' +
          '<button class="btn chico" id="btnReiniciarEscenario">Reiniciar con 26 h de historial nuevo</button>' +
        '</div>';
    }

    html +=
      '<div class="sep"></div>' +
      '<div class="subtitulo">Calibración</div>' +

      '<div class="fila">' +
        '<div class="campo">' +
          '<label>Tara de la bolsa (g)</label>' +
          '<input type="number" id="inTaraDet" value="' + d.tara + '">' +
        '</div>' +
        '<div class="campo">' +
          '<label>Vaciados registrados</label>' +
          '<input type="number" value="' + d.vaciados + '" disabled>' +
        '</div>' +
      '</div>' +
  ...
  function renderEventosCama(cama) {
    var ev = Modelo.estado.eventos
      .filter(function (e) {
        return e.camaId === cama.id;
      })
      .slice(-60)
      .reverse();

    if (!ev.length) {
      U.$('#detEventos').innerHTML =
        '<p class="nota">Sin eventos registrados para esta cama.</p>';
      return;
    }

    var filas = ev.map(function (e) {
      return '<tr>' +
        '<td style="font-family:var(--mono);white-space:nowrap">' + U.fechaHora(e.t) + '</td>' +
        '<td>' + U.esc(e.tipo) + '</td>' +
        '<td>' + U.esc(e.operador || 'Sistema') + '</td>' +
        '<td>' + U.esc(e.texto) + '</td>' +
      '</tr>';
    }).join('');
  ...
  function abrirAsignar(camaId) {
    ...
    Modelo.estado.camas.forEach(function (c) {
      if (c.pacienteId && c.id !== camaId) {
        pacientesUsados[c.pacienteId] = 1;
      }
    });

    var pacientesLibres = Object.keys(Modelo.estado.pacientes)
      .map(function (k) {
        return Modelo.estado.pacientes[k];
      })
      .filter(function (p) {
        return !pacientesUsados[p.id];
      });
    ...
    html +=
      '<div class="campo">' +
        ...
        '<select id="selPaciente">' +
          ...
          pacientesLibres.map(function (p) {
            return '<option value="' + p.id + '"' +
              (cama.pacienteId === p.id ? ' selected' : '') +
              '>' +
              U.esc(p.nombre) +
              ' · ' +
              U.esc(p.hc) +
    ...
    html +=
      '<div class="acciones" style="justify-content:flex-start">' +
        '<button class="btn secundario chico" id="btnNuevoPaciente">+ Nuevo paciente</button>' +
      '</div>';
    ...
    html +=
      '<div class="sep"></div>' +
      '<div class="subtitulo">Dispositivo</div>';

    html += '<div class="lista-dispositivos" id="listaDisp"></div>';

    html +=
      '<div class="acciones" style="justify-content:flex-start">' +
        '<button class="btn secundario chico" id="btnNuevoDisp">+ Dispositivo piloto de prueba</button>' +
      '</div>';

    html +=
      '<div class="sep"></div>' +
      '<div class="acciones">' +
        (cama.dispositivoId || cama.pacienteId
          ? '<button class="btn peligro chico" id="btnLiberar">Liberar cama</button>'
          : '') +
    ...
    function pintarListaDisp() {
      ...
      var actual =
        cama.dispositivoId
          ? Modelo.estado.dispositivos[cama.dispositivoId]
          : null;

      if (
        actual &&
        !lista.some(function (x) {
          return x.id === actual.id;
        })
      ) {
      ...
      cont.innerHTML =
        lista.length
          ? lista.map(function (d) {
              return '<div class="disp-opcion' +
                (dispSel === d.id ? ' sel' : '') +
                '" data-id="' + d.id + '">' +

                '<div>' +
                  '<div class="serie">' + U.esc(d.serie) + '</div>' +
                  '<div class="desc">' +
                    (d.tipo === 'sim'
                      ? 'Piloto · ' + (Simulador.escenarios[d.escenario] || {}).nombre
                      : 'Dispositivo real') +
                  '</div>' +
                '</div>' +

                '<div class="estado-mini">' +
                  (d.estado === 'en-linea' ? 'en línea' : 'sin señal') +
                '</div>' +
              '</div>';
            }).join('')
          : '<p class="nota">No hay dispositivos libres. Se puede crear uno piloto para pruebas.</p>';

      U.$$('.disp-opcion', cont).forEach(function (op) {
        op.onclick = function () {
          dispSel =
            dispSel === op.dataset.id
              ? null
              : op.dataset.id;

          U.$$('.disp-opcion', cont).forEach(function (o) {
            o.classList.toggle('sel', o.dataset.id === dispSel);
          });
        };
      });
    }
    ...
    U.$('#btnNuevoPaciente').onclick = function () {
      toggleFormNuevoPaciente(true);
    };

    U.$('#btnNuevoDisp').onclick = function () {
      var d = Modelo.crearDispositivo({
        tipo: 'sim',
        escenario: 'normal'
      });
    ...
    U.$('#btnGuardarAsig').onclick = function () {
      ...
      if (nuevo) {
        var p = Modelo.altaPaciente(nuevo);
        pacId = p.id;
      }
    ...
    cont.innerHTML =
      '<div class="fila">' +
        '<div class="campo">' +
          ...
          '<input type="text" id="npNombre" placeholder="Apellido, Nombre">' +
        '</div>' +
        '<div class="campo">' +
          '<label>N.º de historia clínica</label>' +
          '<input type="text" id="npHc" placeholder="HC-00000">' +
        '</div>' +
      '</div>' +
      ...
      '<div class="fila-3">' +
        '<div class="campo">' +
          '<label>Edad</label>' +
          '<input type="number" id="npEdad" min="0" max="120">' +
        '</div>' +
        '<div class="campo">' +
          '<label>Sexo</label>' +
          '<select id="npSexo">' +
        ...
        '<div class="campo">' +
          '<label>Peso (kg)</label>' +
          '<input type="number" id="npPeso" min="1" step="0.5" placeholder="70">' +
        '</div>' +
      '</div>' +

      '<div class="campo">' +
        '<label>Diagnóstico / motivo de ingreso</label>' +
        '<input type="text" id="npDx" placeholder="Opcional">' +
      '</div>';
  }
  ...
  function leerFormNuevoPaciente() {
    ...
    return {
      ...
      hc:
        U.$('#npHc').value.trim() ||
        ('HC-' + Math.floor(Math.random() * 90000 + 10000)),
  ...
  function abrirTrasladar(camaId) {
    ...
    U.$('#trasTitulo').textContent =
      'Trasladar a ' + (p ? p.nombre : 'paciente');
    ...
    if (!libres.length) {
      el.innerHTML =
        '<p class="nota">No hay camas libres para trasladar.</p>';
    } else {
      el.innerHTML = libres.map(function (c) {
        return '<div class="disp-opcion" data-cama="' + U.esc(c.id) + '">' +
          '<div>' +
            '<div class="serie">' + U.esc(c.etiqueta) + '</div>' +
            '<div class="desc">Cama libre</div>' +
          '</div>' +
        '</div>';
      }).join('');
  ...
  function abrirConfig() {
    ...
    var html =
          ...
          ? '<p class="nota">' +
              (
                Nube.activo()
                  ...
                  : 'Configurada, estableciendo conexión…'
              ) +
            '</p>'
          : '<p class="nota">Todavía no está configurada — cada computadora guarda sus propios datos por separado. Para compartir pacientes y camas entre todo el equipo, completá <code>FIREBASE_CONFIG</code> en <code>js/nube.js</code> con los datos de un proyecto de Firebase (gratis).</p>'
      ) +
        ...
        '<select id="cfgTipo">' +
          ['websocket', 'http', 'bluetooth', 'serie'].map(function (t) {
            return '<option value="' + t + '"' +
              (enlace.tipo === t ? ' selected' : '') +
              '>' +
              {
                websocket: 'WiFi · WebSocket (recomendado)',
                http: 'WiFi · HTTP (consulta periódica)',
                bluetooth: 'Bluetooth Low Energy',
                serie: 'Cable USB (Web Serial)'
              }[t] +
            '</option>';
          }).join('') +
        '</select>' +
      ...
      '<div class="fila">' +
        '<div class="campo">' +
          ...
          '<input type="text" id="cfgWs" value="' + U.esc(enlace.urlWebSocket) + '">' +
        '</div>' +
        '<div class="campo">' +
          '<label>URL HTTP</label>' +
          '<input type="text" id="cfgHttp" value="' + U.esc(enlace.urlHttp) + '">' +
        '</div>' +
      '</div>' +
      ...
      '<div class="sep"></div>' +
      '<div class="subtitulo">Calibración de la medición</div>' +

      '<div class="fila-3">' +
        '<div class="campo">' +
          '<label>Densidad de la orina</label>' +
          '<input type="number" id="cfgDensidad" step="0.001" value="' + med.densidadOrina + '">' +
        '</div>' +
        '<div class="campo">' +
          '<label>Tara de bolsa (g)</label>' +
          '<input type="number" id="cfgTara" value="' + med.taraBolsaG + '">' +
        '</div>' +
        '<div class="campo">' +
          '<label>Capacidad bolsa (mL)</label>' +
          '<input type="number" id="cfgCap" value="' + med.capacidadBolsaML + '">' +
        '</div>' +
      '</div>' +

      '<div class="sep"></div>' +
      '<div class="subtitulo">Modo piloto</div>' +

      '<div class="campo">' +
        '<label>Velocidad de simulación</label>' +
        '<select id="cfgVel">' +
          CFG.velocidades.map(function (v) {
            return '<option value="' + v.v + '"' +
              (Modelo.estado.velocidad === v.v ? ' selected' : '') +
              '>' +
              v.rot +
            '</option>';
          }).join('') +
      ...
      '<div class="sep"></div>' +
      '<div class="subtitulo">Datos locales</div>' +

      '<p class="nota">Todo se guarda en este navegador (' +
        Almacen.tamano() +
      ...
      '<div class="acciones">' +
        '<button class="btn chico" id="btnGuardarCfg">Guardar configuración</button>' +
      '</div>';
    ...
    U.$('#btnDesconectarCfg').onclick = function () {
      Conexion.desconectar();
    };

    U.$('#btnGuardarCfg').onclick = function () {
      med.densidadOrina =
        parseFloat(U.$('#cfgDensidad').value) ||
        med.densidadOrina;

      med.taraBolsaG =
        parseFloat(U.$('#cfgTara').value) ||
        med.taraBolsaG;

      med.capacidadBolsaML =
        parseFloat(U.$('#cfgCap').value) ||
        med.capacidadBolsaML;

      Modelo.estado.velocidad =
        parseInt(U.$('#cfgVel').value, 10) ||
        1;
    ...
    U.$('#btnBorrarTodo').onclick = function () {
      if (!confirm(
        'Esto borra todos los pacientes, dispositivos e historial guardados en este navegador y recarga la sala de ejemplo. ¿Continuar?'
      )) {
        return;
      }
  ...
  function bind() {
    U.$$('.modo').forEach(function (b) {
      b.onclick = function () {
        Acciones.cambiarModo(b.dataset.modo);
      };
    });
    ...
    U.$('#modalConfig').addEventListener('click', function (ev) {
      if (ev.target.id === 'modalConfig') {
        cerrarConfig();
      }
    });
    ...
    U.$('#cabeceraEventos').onclick = function () {
      ...
      U.$('.panel-eventos').classList.toggle(
        'expandido',
        eventosExpandido
      );
    ...
    U.$('#buscar').oninput = function (ev) {
      filtroTexto = ev.target.value;
      pintarMural();
    };

    U.$('#filtroEstado').onchange = function (ev) {
      filtroEstado = ev.target.value;
      pintarMural();
    };

    U.$('#orden').onchange = function (ev) {
      orden = ev.target.value;
      pintarMural();
    };

    U.$('#btnNuevaCama').onclick = function () {
      Acciones.agregarCama();
    };

    U.$('#btnCerrarDetalle').onclick = cerrarDetalle;

    U.$('#modalDetalle').addEventListener('click', function (ev) {
      if (ev.target.id === 'modalDetalle') {
        cerrarDetalle();
      }
    });
    ...
    U.$$('#rangoTiempo .rango').forEach(function (b) {
      b.onclick = function () {
        rangoHoras =
          parseInt(b.dataset.horas, 10);

        U.$$('#rangoTiempo .rango').forEach(function (x) {
          x.classList.toggle('activo', x === b);
        });
    ...
    U.$('#btnAsignarDesdeDetalle').onclick = function () {
      if (camaAbierta) {
        abrirAsignar(camaAbierta);
      }
    };

    U.$('#btnTrasladar').onclick = function () {
      if (camaAbierta) {
        abrirTrasladar(camaAbierta);
      }
    };
    ...
    U.$('#btnGuardarInventario').onclick = function () {
      ...
      Acciones.guardarSerieInventario(
        camaAbierta,
        U.$('#detSerieInventario').value.trim()
      );
    };

    U.$('#detSerieInventario').addEventListener('keydown', function (ev) {
      if (ev.key === 'Enter') {
        U.$('#btnGuardarInventario').click();
      }
    });

    U.$('#btnCerrarTrasladar').onclick = cerrarTrasladar;

    U.$('#modalTrasladar').addEventListener('click', function (ev) {
      if (ev.target.id === 'modalTrasladar') {
        cerrarTrasladar();
      }
    });

    U.$('#btnVaciarBolsa').onclick = function () {
      if (camaAbierta) {
        Acciones.vaciarBolsa(camaAbierta);
      }
    };
    ...
    U.$('#modalAsignar').addEventListener('click', function (ev) {
      if (ev.target.id === 'modalAsignar') {
        cerrarAsignar();
      }
    });
    ...
    U.$('#modalSilenciar').addEventListener('click', function (ev) {
      if (ev.target.id === 'modalSilenciar') {
        cerrarSilenciar();
      }
    });

    U.$('#btnSilenciarAtendido').onclick = function () {
      confirmarSilenciar('atendido');
    };

    U.$('#btnSilenciarEspera').onclick = function () {
  ...
  return {
    ...
    pintarBarraEstado: pintarBarraEstado,
